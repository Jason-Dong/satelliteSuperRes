...
class Model:
    def __init__(self, model, dataset, model_name, pretrained=False, pretrained_SISRNet=None, pretrained_RegNet=None):
        ...
        self.model = model
        self.dataloader = DataLoader(dataset, batch_size=self.batch_size, shuffle=True, num_workers=8, pin_memory=True)
...
def run():
    ...
    model_wrapper = Model(model, data, 'attempt', pretrained=False)
    model_wrapper.train()
